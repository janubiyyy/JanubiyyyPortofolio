...
import { Code2, Palette, Smartphone, Database, TestTube2, Settings2 } from 'lucide-react';
...
const SkillsSection = () => {
  ...
  const skillCategories = [
    {
      title: language === 'id' ? 'Frontend Technologies' : 'Frontend Technologies',
  ...
  const achievements = language === 'id' ? [
    'Menyelesaikan 5+ proyek web berdampak tinggi yang dipuji karena desain intuitif dan performa',
    ...
    'Delivered 5+ high-impact web projects praised for intuitive design and performance',
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {skillCategories.map((category, categoryIndex) => (
              <div key={categoryIndex} className="bg-card border border-border rounded-xl p-6 shadow-lg animate-fade-in hover:shadow-xl transition-all duration-300">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {category.skills.map((skill, skillIndex) => (
                    <div key={skillIndex} className="flex items-center space-x-2 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200 border border-border/50">
                      <span className={`text-sm font-bold ${skill.color}`}>{skill.icon}</span>
